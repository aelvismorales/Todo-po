...
export function show_message(message_loader,message,type){
  let div;
  if (type === SUCCESS){
    div = create_div_alert('alert-3','flex items-center p-4 mb-4 text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400',message);
  } else if (type=== ERROR){
    div = create_div_alert('alert-2','flex items-center p-4 mb-4 text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400',message);

  }else{
    div = create_div_alert('alert-4','flex items-center p-4 mb-4 text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-400',message);
  }
...
const modal_close_footer_btn = document.getElementById('modal_close_footer_btn');

// Open modal
btn_crear_task_list.addEventListener('click', () => {
  open_modal(modal_crear_task_list);
});

// Close modal
modal_crear_task_list.addEventListener('click', (e) => {
  if (e.target === modal_crear_task_list || e.target === modal_close_svg || e.target === modal_close_footer_btn) {
...
save_button.addEventListener('click', async (e) => {
  ...
  const message_loader_form = document.getElementById('message_loader_crear_modal');
  ...
  try {
    ...
    if (response.ok){
      show_message(message_loader_form,'Task list created successfully','sucess');
      close_modal(modal_crear_task_list);
      save_button.disabled = false;
      form.reset();
