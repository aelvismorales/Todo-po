...
import { open_modal, close_modal,show_message } from './modal_create_task_list.js';
...
function populate_update_form(id_task_list){
    ...
    const actual_task_list = document.getElementById(`task_list_item_${id_task_list}`);
    // Get form to update
...
function update_task_list_ui(data,id_task_list){
    ...
    const actual_task_list = document.getElementById(`task_list_item_${id_task_list}`);
    const name_article = document.getElementById(`h_article_task_${id_task_list}`);
    const description_article = document.getElementById(`description_task_${id_task_list}`);
...
let btn_update_task_list = document.querySelectorAll('#btn_update_task_list');
const modal_update_task_list = document.getElementById('modal_update_task_list');

btn_update_task_list.forEach((btn)=>{
    const id = btn.getAttribute('data-id');
    btn.addEventListener('click', ()=> open_update_modal(id));
})
...
update_save_button.addEventListener('click', async (e) => {
    ...
    const message_loader_form = document.getElementById('message_loader_update_modal');
...
const modal_close_update_svg = document.getElementById('modal_close_update_svg');
const modal_close_update_footer_btn = document.getElementById('modal_close_update_footer_btn');

// Add event listener to close modal update task list
modal_update_task_list.addEventListener('click', (e) => {
    if (e.target === modal_update_task_list || e.target === modal_close_update_svg || e.target === modal_close_update_footer_btn) {
        close_modal(modal_update_task_list);
    }
});
